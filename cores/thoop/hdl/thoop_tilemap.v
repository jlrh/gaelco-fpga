...
//    -1 : registra (tmx,tmy)
//     0 : tile_a = {layer, ty, tx}              (lee el par {data,data2} de thoop_vmem)
...
module thoop_tilemap (
    ...
    // puerto de lectura de tile de thoop_vmem (par {data, data2}), latencia 1 ce
    ...
    // (El sim DEBE descargar el blob desde el romset RAW, no thoop_di; si no, doble de-interleave -> glifos.)
    ...
    // paridad del tile (tx[0]=tmx_r[4]) REGISTRADA para alinear con col0_d/col1 (que van registrados desde
    // tmx_r): sin el registro wpar iria 1 ciclo ADELANTADO respecto a col1[3] -> 1px del borde leeria el
    // banco equivocado.
    reg wpar;
