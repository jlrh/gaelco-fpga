// ============================================================================
//  Thunder Hoop (Gaelco, 1992) — jtthoop_game.v: modulo de juego para jtframe.
//
//  jtcore (memgen, desde cfg/mem.yaml) AUTO-GENERA jtthoop_game_sdram.v (GAMETOP) que
...
module jtthoop_game(
    ...
    // ===================== TIMING DE VIDEO (dentro de thoop_video_top) =====================
    wire vblank_irq;
    wire hs_w, vs_w, hb_w, vb_w, de_w;

    // ===================== ENTRADAS =====================
    wire [15:0] in_dsw2, in_dsw1, in_p1, in_p2;
    thoop_inputs u_inputs (
        .dipsw(dipsw[15:0]),
        .joystick1(joystick1[5:0]), .joystick2(joystick2[5:0]),
        .coin(coin[1:0]),         // jtframe coin ya activo-bajo; thoop_inputs espera activo-bajo -> DIRECTO
    ...
    // ACTIVO-BAJO y thoop_inputs trabaja activo-bajo -> TODO DIRECTO. El `~service` previo metia DSW2 bit7=0
    // = MODO SERVICIO permanente (el juego no arrancaba). Ver squash + research/BITACORA_THOOP.md.

    // ===================== CPU + memoria (thoop_main) =====================
    wire        flip_screen;
