...
module thoop_main (
    ...
    // --- puertos de entrada (ya ensamblados por thoop_inputs) ---
    input  wire [15:0] in_dsw2, in_dsw1, in_p1, in_p2,

    // --- puerto CPU hacia thoop_vmem ---
    ...
    // registrada -> infiere block-RAM (mismo patron que thoop_vmem).
...
`ifdef SIMULATION
    ...
    // VOLCADO de work RAM en frames clave para DIFF vs MAME (mame_wram_thoop_{260,400}.bin).
    reg vbl_e=0; reg [15:0] frm=0; reg dB=0,dC=0;
    always @(posedge clk) begin
        vbl_e <= vblank_irq;
        if (vblank_irq & ~vbl_e) begin
            frm <= frm + 1'b1;
            if (frm==16'd260 && !dB) begin dB<=1'b1; $writememh("our_wram_thoop_hi_260.hex",wram_hi); $writememh("our_wram_thoop_lo_260.hex",wram_lo); $display("WRAMDUMP thoop 260 pc=%h", {prog_addr,1'b0}); end
            if (frm==16'd400 && !dC) begin dC<=1'b1; $writememh("our_wram_thoop_hi_400.hex",wram_hi); $writememh("our_wram_thoop_lo_400.hex",wram_lo); $display("WRAMDUMP thoop 400 DONE pc=%h", {prog_addr,1'b0}); $finish; end
        end
    end
`endif
