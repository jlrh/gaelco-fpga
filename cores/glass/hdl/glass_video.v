// ============================================================================
//  Thunder Hoop (Gaelco) — glass_video.v: compositor de los 2 tilemaps (Tipo-1).
//
//  Instancia 2 glass_tilemap (L0, L1) con scroll aplicado y resuelve la PRIORIDAD
...
module glass_video (
    ...
    // glass.cpp screen_update: pant0 sx=vregs[1]+4 sy=vregs[0]; pant1 sx=vregs[3] sy=vregs[2].
    // +16 vertical = visarea Y de MAME empieza en 16 (screen_y = vpos+16 -> tm_y = screen_y + scrolly).
...
`endif
`ifdef GLASS_L1TRACE
    // DIAG: traza el BORDE DERECHO (vpos=144, hpos 372..388) -> por que x=367 sale negro.
    always @(posedge clk) if (ce && vpos==9'd144 && hpos>=9'd372 && hpos<=9'd388) begin
        $display("RE hpos=%0d pen0=%h col0=%h op0=%b pen1=%h col1=%h op1=%b l0win=%b idx=%h tmx0=%h",
                 hpos, pen0, color0, op0, pen1, color1, op1, l0_wins, (op0&l0_wins)?{color0,pen0}:(op1?{color1,pen1}:10'd0), tmx0);
    end
`endif
`ifdef SIMULATION
