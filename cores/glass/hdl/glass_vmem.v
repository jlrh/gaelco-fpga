...
module glass_vmem (
    ...
    input  wire [10:0] tile_a0,
    output reg  [31:0] tile_q0,
    input  wire [10:0] tile_a1,
    output reg  [31:0] tile_q1,
    input  wire [9:0]  pal_a,
    output reg  [15:0] pal_q,
    input  wire [9:0]  palb_a,
    output reg  [15:0] palb_q,
    input  wire [9:0]  palc_a,
    output reg  [15:0] palc_q,
    input  wire [10:0] spr_a,
    output reg  [15:0] spr_q,
    input  wire        scene_dump
);

    reg [7:0] e0_hi[0:2047], e0_lo[0:2047], o0_hi[0:2047], o0_lo[0:2047];
    reg [7:0] e1_hi[0:2047], e1_lo[0:2047], o1_hi[0:2047], o1_lo[0:2047];
    reg [7:0] ec_hi[0:2047], ec_lo[0:2047], oc_hi[0:2047], oc_lo[0:2047];
    wire [11:0] vwo = cpu_addr[12:1];
    wire [10:0] vk  = vwo[11:1];
    wire        vpar= vwo[0];
    ...
    reg [7:0] pv_hi[0:1023], pv_lo[0:1023];
    reg [7:0] pb_hi[0:1023], pb_lo[0:1023];
    reg [7:0] pd_hi[0:1023], pd_lo[0:1023];
    reg [7:0] pc_hi[0:1023], pc_lo[0:1023];
    wire [9:0] pwo = cpu_addr[10:1];
    wire pw_hi = cpu_we & cs_pal & cpu_uds;
    wire pw_lo = cpu_we & cs_pal & cpu_lds;
    always @(posedge clk) begin
        if (pw_hi) begin pv_hi[pwo]<=io_wdata[15:8]; pb_hi[pwo]<=io_wdata[15:8]; pd_hi[pwo]<=io_wdata[15:8]; pc_hi[pwo]<=io_wdata[15:8]; end
        if (pw_lo) begin pv_lo[pwo]<=io_wdata[7:0];  pb_lo[pwo]<=io_wdata[7:0];  pd_lo[pwo]<=io_wdata[7:0];  pc_lo[pwo]<=io_wdata[7:0];  end
    end
    always @(posedge clk) if (ce_pix) pal_q  <= {pv_hi[pal_a],  pv_lo[pal_a]};
    always @(posedge clk) if (ce_pix) palb_q <= {pb_hi[palb_a], pb_lo[palb_a]};
    always @(posedge clk) if (ce_pix) palc_q <= {pd_hi[palc_a], pd_lo[palc_a]};
    always @(posedge clk) cpu_pal_rdata <= {pc_hi[pwo], pc_lo[pwo]};

    reg [7:0] qv_hi[0:2047], qv_lo[0:2047];
    reg [7:0] qc_hi[0:2047], qc_lo[0:2047];
...
`ifdef GLASS_SCENE
    ...
    initial begin
        ...
        $readmemh("scene_pal_hi.hex", pd_hi); $readmemh("scene_pal_lo.hex", pd_lo);
        ...
        $display("GLASS_SCENE: VRAM/paleta/spriteRAM precargadas (replay)");
    end
`endif
`ifdef SIMULATION
    ...
    always @(posedge clk) if (scene_dump) begin
        ...
        $display("GLASS_SCENE DUMP: escena volcada a scene_*.hex");
    end
`endif
endmodule
