...
//    -1 : registra (tmx,tmy)
//     0 : tile_a = {layer, ty, tx}              (lee el par {data,data2} de thoop2_vmem)
...
module thoop2_tilemap (
    ...
    // puerto de lectura de tile de thoop2_vmem (par {data, data2}), latencia 1 ce
    output wire [10:0] tile_a,      // {layer, tile_index}  (tile_index = ty*32+tx)
    input  wire [31:0] tile_q,      // {data[31:16], data2[15:0]}

    // puerto de lectura ROM gfx (4 planos DW32), latencia variable con handshake. 21b = gfx 8MB DW32.
    output wire [20:0] rom_a,      // indice de WORD DW32 (0..2M-1)
    ...
    // thoop2 get_tile_info: code=((data&0xfffc)>>2)|((data&3)<<14) (16b, SIN base 0x4000);
    //   color=data2[5:0], categoria=data2[7:6], flip = data2[15:14] (NO en data/w0 como thoop).
    wire [15:0] code  = {data[1:0], data[15:2]};
    wire        flipx = data2[14];
    wire        flipy = data2[15];
    ...
    // rom_a = indice WORD DW32 = code*32 + (col>=8?16:0) + row (code 16b -> code*32 cabe en 21b).
    // two-halves: SIN de-interleave [0,2,1,3] (eso era de RGN_FRAC(1,4); el golden confirmó h8/h12 directo).
    wire [20:0] rom_a_lin = {code, 5'b00000} + {16'b0, col1[3], row1};
