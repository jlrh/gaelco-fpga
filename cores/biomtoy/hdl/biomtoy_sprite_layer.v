...
`endif
    ...
    wire [12:0] lb_raw;
    assign lb_q = rst ? 13'd0 : lb_raw;

    biomtoy_sprite_engine u_spr (
        .clk(clk), .rst(rst), .ce(1'b1), .start(start), .line(next_vpos + YOFFS[8:0]), .busy(busy),
        .spr_a(spr_a), .spr_q(spr_q),
        .rom_a(rom_a), .d_p0(d_p0), .d_p1(d_p1), .d_p2(d_p2), .d_p3(d_p3), .gfx_ok(gfx_ok),
        .lb_x(hpos), .lb_q(lb_raw),
