// ============================================================================
//  Squash (Gaelco) — "PLACA" sintetizable: 68000 (fx68k) + mapa de memoria +
...
module squash_main (
    ...
    // --- puertos de entrada (ya ensamblados por squash_inputs) ---
    input  wire [15:0] in_dsw2, in_dsw1, in_p1, in_p2,

    // --- puerto CPU hacia squash_vmem ---
    ...
    assign vmem_dec_wdata = dec_word;     // videoram/screenram se escriben DESCIFRADAS
    ...
    // registrada -> infiere block-RAM (mismo patron que squash_vmem).
...
`ifdef SIMULATION
    ...
    // VOLCADO de work RAM en frames clave (= MAME 191/260/320) para DIFF vs oraculo.
    reg vbl_e=0; reg [15:0] frm=0; reg dA=0,dB=0,dC=0;
    always @(posedge clk) begin
        vbl_e <= vblank_irq;
        if (vblank_irq & ~vbl_e) begin
            frm <= frm + 1'b1;
            if (frm==16'd320 && !dA) begin dA<=1'b1; $writememh("our_wram_hi_320.hex",wram_hi); $writememh("our_wram_lo_320.hex",wram_lo); $display("WRAMDUMP 320 pc=%h", {prog_addr,1'b0}); end
            if (frm==16'd460 && !dB) begin dB<=1'b1; $writememh("our_wram_hi_460.hex",wram_hi); $writememh("our_wram_lo_460.hex",wram_lo); $display("WRAMDUMP 460 pc=%h", {prog_addr,1'b0}); end
            if (frm==16'd690 && !dC) begin dC<=1'b1; $writememh("our_wram_hi_690.hex",wram_hi); $writememh("our_wram_lo_690.hex",wram_lo); $display("WRAMDUMP 690 DONE pc=%h  TRIP 18c4=%b 19d4=%b 19fc=%b", {prog_addr,1'b0}, hit_18c4, hit_19d4, hit_19fc); $finish; end
        end
    end
    // TRIPWIRE: ¿nuestro 68k EJECUTA la rutina del countdown ffb925? (MAME: PC 0x18C4/0x19D4/0x19FC)
    reg hit_18c4=0, hit_19d4=0, hit_19fc=0;
    always @(posedge clk) begin
        if (prog_addr==19'h00C62) hit_18c4 <= 1'b1;   // 0x18C4
        if (prog_addr==19'h00CEA) hit_19d4 <= 1'b1;   // 0x19D4
        if (prog_addr==19'h00CFE) hit_19fc <= 1'b1;   // 0x19FC
    end
    // LOG de escrituras a ffb924/25 (el countdown del check) con PC — espejo del trace de MAME.
    always @(posedge clk) begin
        if ((ww_hi|ww_lo) && wramidx==15'h5c92)
            $display("OURFFB925W pc=%h data=%h hi=%b lo=%b", {prog_addr,1'b0}, oEdb, ww_hi, ww_lo);
    end
    // TRIPWIRE INIT: ¿llega el 68k al init que fija el estado (PC 0x560/0x562) y al read de DSW (0x58E)?
    // + LOG de lo que NUESTRO core devuelve en DSW1/DSW2 (golden MAME: DSW2=0xDF, DSW1=0xFF).
    reg hit_560=0, hit_562=0, hit_58e=0, dsw2_logged=0, dsw1_logged=0; reg [7:0] n_io=0;
    always @(posedge clk) begin
        if (prog_addr==19'h002B0) hit_560 <= 1'b1;   // 0x560
        if (prog_addr==19'h002B1) hit_562 <= 1'b1;   // 0x562
        if (prog_addr==19'h002C7) hit_58e <= 1'b1;   // 0x58E
        if (cs_dsw2 & rw_rd & ~dsw2_logged) begin dsw2_logged<=1'b1; $display("OURDSW2 rd=%h pc=%h", in_dsw2, {prog_addr,1'b0}); end
        if (cs_dsw1 & rw_rd & ~dsw1_logged) begin dsw1_logged<=1'b1; $display("OURDSW1 rd=%h pc=%h", in_dsw1, {prog_addr,1'b0}); end
        // GROUND TRUTH: lectura en la region I/O 0x70xxxx (addr[23:20]==7) con addr+dato+PC (capado a 60).
        if (as_rising & rw_rd & (addr[23:20]==4'h7) & (n_io<8'd60)) begin
            n_io <= n_io + 8'd1;
            $display("OURIO rd addr=%h cs(d2,d1,p1,p2)=%b%b%b%b iEdb=%h pc=%h", addr, cs_dsw2,cs_dsw1,cs_p1,cs_p2, iEdb, {prog_addr,1'b0});
        end
        if (dc[19:0]==0) $display("INITTRIP 560=%b 562=%b 58e=%b dsw2log=%b dsw1log=%b", hit_560, hit_562, hit_58e, dsw2_logged, dsw1_logged);
    end
`endif
