// ============================================================================
//  Squash (Gaelco, 1992) — jtsquash_game.v: modulo de juego para jtframe.
//
//  jtcore (memgen, desde cfg/mem.yaml) AUTO-GENERA jtsquash_game_sdram.v (GAMETOP) que
...
module jtsquash_game(
    ...
    // ===================== TIMING DE VIDEO (dentro de squash_video_top) =====================
    wire vblank_irq;
    wire hs_w, vs_w, hb_w, vb_w, de_w;

    // ===================== ENTRADAS =====================
    wire [15:0] in_dsw2, in_dsw1, in_p1, in_p2;
    squash_inputs u_inputs (
        .dipsw(dipsw[15:0]),
        .joystick1(joystick1[5:0]), .joystick2(joystick2[5:0]),
        .coin(coin[1:0]),         // jtframe coin ya activo-bajo; squash_inputs espera activo-bajo -> DIRECTO
    ...
    // ACTIVO-BAJO (0=pulsado) y squash_inputs trabaja en activo-bajo -> TODO DIRECTO (sin invertir).
    ...
    // ===================== SNAPSHOT de escena (SQUASH_SCENE) — iteracion rapida del video =====================
    // SQUASH_SCENE_DUMP=N : corre normal y al frame N vuelca VRAM/paleta/spriteRAM/vregs a scene_*.hex.
    // SQUASH_SCENE        : mantiene la CPU en RESET y precarga la escena -> renderiza sin bootear (segundos).
    wire        scene_dump;
...
`ifdef SIMULATION
  ...
  `ifdef SQUASH_SCENE_DUMP
    assign scene_dump = (scene_fcnt==`SQUASH_SCENE_DUMP) & vblank_irq & ~vbi_sd;
